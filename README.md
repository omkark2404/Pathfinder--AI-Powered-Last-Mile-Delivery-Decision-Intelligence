...
- **License**: CC BY 4.0
- **Committed Benchmark Fixture**: `data/mendeley_planned_vs_actual.csv` (30 routes, 603 stops).
- **Contents**: Synthetic benchmark fixture strictly matching the Mendeley schema (planned route sequences vs actual driver sequences, travel distance, duration, time-window information) for offline CI and test suite execution.
- **Usage in this system**: Route deviation prediction, driver adherence analytics, Kendall Tau similarity index.
- **Production Setup**: To train against the authentic Mendeley research dataset, download from the DOI link above and place at `./data/mendeley_planned_vs_actual.csv`.
...
- **CORS**: Configurable via `ALLOWED_ORIGINS` environment variable (default `*` is strictly for local development; set explicit frontend domain e.g. `https://your-domain.com` in production)
